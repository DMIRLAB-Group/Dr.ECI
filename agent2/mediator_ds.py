...
input_file_path = './output/explorer_choose.jsonl'
...
for index, item in tqdm(enumerate(representative_cots_data[:2]), desc="Processing items", unit=" item"):
    try:
        ...
        mediator_system = prompt.mediator_system
        mediator = prompt.mediator(source_event, target_event, text)


        # 构建messages
        messages = [
            {"role": "system", "content": mediator_system},
            {"role": "user", "content": mediator}
        ]
        ...
        #     {"role": "user", "content": "#########\n" +mediator + "\n##########\n"+ response_text_round + "\n\n###########\n" + prompt.mediator_formal}
        # ]
        # print(f"Messages Round 2: {messages2}")
        # # messages.append({"role": "user", "content": ans_formal})
        ...
        results.append({
            ...
            "events": item["events"],
            'mediator_ans': response_text_round
...
output_file_path = './output/mediator_ds.jsonl'
